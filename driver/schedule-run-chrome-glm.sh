...
if [ -e /tmp/awfy-daemon-chrome-glm ]
then
  echo "awfy: Already running"
  exit 0
fi

touch /tmp/awfy-daemon-chrome-glm
...
#        popd
        ...
        pushd /home/user/work/chromium_glm_repos/chromium/src
        git fetch
        ...
        if [ -z "$list" ]; then
            ...
            do
                ...
                if [[ -n $v8find ]]; then
                    ...
                    git reset --hard $i
                    pushd /home/user/work/awfy/driver
                    python dostuff-chrome-glm.py  --config=client/machine_config/electro-glm.config
