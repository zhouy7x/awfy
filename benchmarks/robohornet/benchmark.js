...
//print("execute ---  bench name: "+bench[name]+"  \n");
//print("execute ---  bench name: "+name+" bench _original is:"+bench._original+"   \n");
      // execute method
...
//print("invoke ---  start  -- 1\n");

    // start iterating over the array
    if (raiseIndex() !== false) {
//print("invoke ---  start  -- 2\n");
      // emit "start" event
      bench = result[index];
...
//print("invoke ---  start  -- 3\n");
      ...
      // else start
        ...
        if (isAsync(bench)) {
//print("invoke ---  start  -- 4\n");
          delay(bench, execute);
        } else {
//print("invoke ---  start  -- 5\n");
          while (execute()) { }
        }
      }
    }
//print("invoke ---  start  -- 6\n");
    return result;
  }
...
//print("clock:  -------- clone.error is:"+clone.error+"-------------  1 \n");
...
//      };
      ...
      var source = {
        'setup': getSource(bench.setup, preprocess('m$.setup(' + fnArg +')')),
        'fn': getSource(fn, preprocess('m$.fn(' + fnArg + ')')),
        'fnArg': fnArg,
        'teardown': getSource(bench.teardown, preprocess('m$.teardown()'))
      };
...
//print("clock:  -------- clone.error is:"+clone.error+"-------------  2 \n");
...
//print("clock:  -------- clone.error is:"+clone.error+"-------------  3 \n");
      ...
      var compiled = bench.compiled = createFunction(preprocess('t$'), interpolate(
        preprocess(deferred
          ...
          : 'var r$,s$,m$=this,arg$=m$.name,su$=m$.setup,f$=m$.fn,i$=m$.count,n$=t$.ns;{su$(arg$[0], arg$[1])}\n#{begin};' +
            'while(i$--){f$(arg$[0], arg$[1])\n}#{end};#{teardown}\nreturn{elapsed:r$,uid:"#{uid}"}'),
      //    : 'var r$,s$,m$=this,arg$=m$.name,su$=m$.setup,f$=m$.fn,i$=m$.count,n$=t$.ns;{print("arg is:"+arg$);}\n{su$(arg$)}\n#{begin};' +
      //      'while(i$--){#{fn}\n}#{end};#{teardown}\nreturn{elapsed:r$,uid:"#{uid}"}'),
      // Old code
...
// print("clock:  -------- compiled is:"+compiled+"-------------  4 \n");
...
// print("clock:  -------- clone.error is:"+clone.error+"-------------  4 \n");
...
//print("clock:  -------- clone.error is:"+clone.error+"-------------  5 \n");
        try {
      ...
      if (!clone.error) {
//print("clock:  -------- clone.error is:"+clone.error+"-------------  6 \n");
        result = compiled.call(deferred || bench, timer).elapsed;
      }
//print("clock:  -------- clone.error is:"+clone.error+"-------------  7 \n");
      return result;
    };
    ...
    function getRes(unit) {
      ...
      while (count--) {
        ...
        if (measured > 0) {
// print("getRes: measured is: "+measured+"   ---\n");
          sample.push(measured);
        } else {
// print("getRes: measured is: infinity --\n");
...
//print("compute: sample.length is:"+sample.length+"  ---\n");
    /**
     * Adds a clone to the queue.
     */
    function enqueue() {
//print("compute:enqueue _original is:"+bench+"  ---\n");
...
//print("evaluate:   size is: "+size+"   clone.times.period is:"+clone.times.period+"  --\n");
//print("evaluate:   sample len is: "+sample.length+"  --\n");
      ...
      if (!done) {
        ...
        rme = (moe / mean) * 100 || 0;
//print("evaluate:   mean time is: "+mean+"  --\n");
        extend(bench.stats, {
          'deviation': sd,
          'mean': mean,
          'moe': moe,
    ...
    enqueue();
//print("compute: call invoke run ------------111111111--- \n");
    invoke(queue, {
      'name': 'run',
      'args': { 'async': async },
      'queued': true,
...
//print("cycle: clone.running is:"+clone.running+"   -- \n");
    options || (options = {});
...
//print("cycle: clocked is:"+clocked+" -- \n");
...
//print("cycle: period is:"+period+"    clocked is:"+(clocked)+"    count is:"+count+"   -- \n");
      // ops per second
...
//print("run: me._original is:"+me._original+"  -------------------------------\n");
    if (!event.cancelled) {
...
//print("run: call cycle   -------------------------------\n");
          cycle(me, options);
        }
      }
      // for original benchmarks
      else {
//print("run: call compute   ----------------------------\n");
        compute(me, options);
      }
    }
    return me;
