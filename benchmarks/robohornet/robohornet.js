...
(function() {
  ...
  _p.next_ = function() {
    ...
    if (benchmark) {
      ...
      // zxli add here to start the testing 
      this.benchmarkLoaded();
    } else {
      this.done_();
    }
  };
...
//    }
  ...
  _p.benchmarkLoaded = function() {
    ...
    for (var run, i = 0; run = benchmark.runs[i]; i++) {
      var arg =[benchmark.filename, run[1]];
//print("run _p.benchmarkLoaded: run0[0]: "+run[0]+"  run[1]= "+run[1]+"  \n");
      //suite.add(run[0], {
      suite.add(arg, {
        //defer: true,
        defer: false,
//        fn: bind(callTest, suite, arg),
//        setup: bind(callFunction, suite, 'setUp', arg),
//        teardown: bind(callFunction, suite, 'tearDown', arg),
//        reset: bind(callFunction, suite, "reset", arg)
       fn: function(n0, n1) {
           if(n0 == 'object_scope_')
             object_scope_test(n1);
           else if(n0 == 'property_accessors_')
             property_accessors_test(n1);
           else if(n0 == 'varargs_')
             varargs_test(n1);
        },
       setup: function(n0, n1) {
           //setUp(n);
//           print("setup: n0 is: "+n0+"   n1 is:"+n1+"   --\n");
           if(n0 == 'object_scope_')
             object_scope_setUp(n1);
           else if(n0 == 'property_accessors_')
             property_accessors_setUp(n1);
           else if(n0 == 'varargs_')
             varargs_setUp(n1);
...
//print("run _p.benchmarkLoaded ---- start \n");
    suite.run(true);
  };
...
//      this.benchmarksById_[benchmark.id] = benchmark;
    ...
    this.benchmarksById_ = {};
    for (var details, i = 0; details = benchmarks[i]; i++) {
//print("_p.initBenchmarks_: benchmark "+i+"  is loaded --\n");
      benchmark = new robohornet.Benchmark(details);
      benchmark.index = i;
      benchmark.computedWeight = details.weight;
...
//print("_p.onBenchmarkComplete_: name is: "+run.name+"  mean time is: "+(run.stats.mean * 1000)+"  run len is: "+run.stats.sample.length +"  --\n");
    }
    benchmark.results = results;
    this.setBenchmarkStatus_(benchmark, robohornet.enabledBenchmarks.SUCCESS);
    this.showBenchmarkResults_(benchmark);
//print("start next benchmark testing --------\n");
    this.next_();
...
//print("_p.showBenchmarkResults_: baselinetime is:"+benchmark.baselineTime+"   computedWeight is:"+benchmark.computedWeight+"  accumulatedMean is:"+accumulatedMean+"   --\n");
...
//print("_p.showBenchmarkResults_: index is:"+score+"   raw score is:"+rawScore+"   --\n");
//print("_p.showBenchmarkResults_: total index is:"+this.score_+"   total raw score is:"+this.rawScore_+"   --\n");
