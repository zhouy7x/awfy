...
(function(exports) {
    ...
    function is_identifier(name) {
        if (typeof name !== "string" || RESERVED_WORDS(name)) return false;
        return true;
    }
    ...
    function is_identifier_char(ch) {
        var code = ch.charCodeAt(0);
        return UNICODE.ID_Continue.test(ch) || code == 36 || code == 95 || code == 8204 || code == 8205;
    }
    ...
    function tokenizer($TEXT, filename, html5_comments, shebang) {
        ...
        function token(type, value, is_comment) {
            S.regex_allowed = type == "operator" && !UNARY_POSTFIX(value) || type == "keyword" && KEYWORDS_BEFORE_EXPRESSION(value) || type == "punc" && PUNC_BEFORE_EXPRESSION(value) || type == "arrow";
            if (type == "punc" && value == ".") {
                prev_was_dot = true;
            } else if (!is_comment) {
                prev_was_dot = false;
            ...
            switch (ch.charCodeAt(0)) {
              ...
              case 117:
                if (peek() == "{") {
                    ...
                    if (length > 6 || (result = hex_bytes(length)) > 1114111) {
    ...
    function parse($TEXT, options) {
        ...
        function _yield_expression() {
            ...
            if (can_insert_semicolon() || is("punc") && PUNC_AFTER_EXPRESSION(S.token.value)) {
    ...
    SymbolDef.prototype = {
        unmangleable: function(options) {
            if (!options) options = {};
            return this.global && !options.toplevel || this.export || this.undeclared || !options.eval && (this.scope.uses_eval || this.scope.uses_with) || options.keep_fnames && (this.orig[0] instanceof AST_SymbolLambda || this.orig[0] instanceof AST_SymbolDefun) || this.orig[0] instanceof AST_SymbolMethod || options.keep_classnames && (this.orig[0] instanceof AST_SymbolClass || this.orig[0] instanceof AST_SymbolDefClass);
        },
    ...
    AST_Toplevel.DEFMETHOD("figure_out_scope", function(options) {
        ...
        var tw = new TreeWalker(function(node, descend) {
            ...
            } else if (node instanceof AST_SymbolVar || node instanceof AST_SymbolLet || node instanceof AST_SymbolConst) {
                var def;
            ...
            if (!(scope instanceof AST_Toplevel) && (node instanceof AST_Export || node instanceof AST_Import)) {
                js_error(node.TYPE + " statement may only appear at top level", node.start.file, node.start.line, node.start.col, node.start.pos);
            }
    ...
    (function() {
        ...
        DEFPRINT(AST_ObjectKeyVal, function(self, output) {
            ...
            if (allowShortHand && self.value instanceof AST_Symbol && is_identifier_string(self.key) && get_name(self.value) === self.key && is_identifier(self.key)) {
                print_property_name(self.key, self.quote, output);
            } else if (allowShortHand && self.value instanceof AST_DefaultAssign && self.value.left instanceof AST_Symbol && is_identifier_string(self.key) && get_name(self.value.left) === self.key) {
                print_property_name(self.key, self.quote, output);
                output.space();
                output.print("=");
                output.space();
    ...
    (function() {
        ...
        (function(def) {
            ...
            function is_modified(tw, node, value, level, immutable) {
                ...
                if (is_lhs(node, parent) || !immutable && parent instanceof AST_Call && parent.expression === node && !(value instanceof AST_Arrow) && !(value instanceof AST_Class) && (!(value instanceof AST_Function) || !(parent instanceof AST_New) && value.contains_this())) {
                    return true;
            ...
            function mark_escaped(tw, d, scope, node, value, level, depth) {
                ...
                if (parent instanceof AST_Assign && parent.operator == "=" && node === parent.right || parent instanceof AST_Call && node !== parent.expression || parent instanceof AST_Exit && node === parent.value && node.scope !== d.scope || parent instanceof AST_VarDef && node === parent.value || parent instanceof AST_Yield && node === parent.value && node.scope !== d.scope) {
                    if (depth > 1 && !(value && value.is_constant_expression(scope))) depth = 1;
                    if (!d.escaped || d.escaped > depth) d.escaped = depth;
                    return;
                } else if (parent instanceof AST_Array || parent instanceof AST_Await || parent instanceof AST_Binary && lazy_op(parent.operator) || parent instanceof AST_Conditional && node !== parent.condition || parent instanceof AST_Expansion || parent instanceof AST_Sequence && node === parent.tail_node()) {
            ...
            def(AST_SymbolRef, function(tw, descend, compressor) {
                ...
                if (d.fixed === undefined || !safe_to_read(tw, d) || d.single_use == "m") {
                    ...
                    if (value instanceof AST_Lambda && recursive_ref(tw, d)) {
                        ...
                        d.single_use = value instanceof AST_Lambda || value instanceof AST_Class || d.scope === this.scope && value.is_constant_expression();
                    } else {
                        d.single_use = false;
                    }
            ...
            function collapse(statements, compressor) {
                    ...
                    if (is_func_expr(fn) && !fn.name && !fn.uses_arguments && !fn.uses_eval && (iife = compressor.parent()) instanceof AST_Call && iife.expression === fn && all(iife.args, function(arg) {
                        ...
                        for (var i = len; --i >= 0; ) {
                            ...
                            if (sym instanceof AST_Expansion) {
                                ...
                                if (!arg) arg = make_node(AST_Undefined, sym).transform(compressor); else if (has_overlapping_symbol(fn, arg, fn_strict)) arg = null;
                ...
                function side_effects_external(node, lhs) {
                    if (node instanceof AST_Assign) {
                        return side_effects_external(node.left, true) || side_effects_external(node.right);
                    }
                    ...
                    if (lhs) {
                        ...
                        if (node instanceof AST_Sub) {
                            return side_effects_external(node.expression, true) || side_effects_external(node.property);
                        }
        ...
        AST_Scope.DEFMETHOD("make_var_name", function(prefix) {
            ...
            prefix = prefix.replace(/[^a-z_$]+/gi, "_");
            var name = prefix;
        ...
        AST_Scope.DEFMETHOD("hoist_properties", function(compressor) {
            ...
            var tt = new TreeTransformer(function(node, descend) {
                ...
                if (node instanceof AST_VarDef) {
                    ...
                    if (sym.scope === self && (def = sym.definition()).escaped != 1 && !def.single_use && !def.direct_access && !compressor.exposed(def) && !top_retain(def) && (value = sym.fixed_value()) === node.value && value instanceof AST_Object) {
                        descend(node, this);
        ...
        OPT(AST_Call, function(self, compressor) {
                ...
                if (all(self.args, function(x) {
                    ...
                    try {
                        ...
                        if (fun.body instanceof AST_Node) {
                            fun.body = [ make_node(AST_Return, fun.body, {
                                value: fun.body
                            }) ];
                        }
            ...
            if (is_func && !fn.is_generator && !fn.async) {
                ...
                if (compressor.option("inline") && simple_args && !fn.uses_arguments && !fn.uses_eval && !(fn.name && fn instanceof AST_Function) && (value = can_flatten_body(stat)) && (exp === fn || compressor.option("unused") && (def = exp.definition()).references.length == 1 && !recursive_ref(compressor, def) && fn.is_constant_expression(exp.scope)) && !self.pure && !fn.contains_this() && can_inject_symbols()) {
        ...
        OPT(AST_SymbolRef, function(self, compressor) {
            ...
            if (compressor.option("reduce_vars") && is_lhs(self, compressor.parent()) !== self) {
                ...
                if (single_use && (fixed instanceof AST_Lambda || fixed instanceof AST_Class)) {
                    if (d.scope !== self.scope && (!compressor.option("reduce_funcs") && fixed instanceof AST_Lambda || d.escaped == 1 || fixed.inlined)) {
                        single_use = false;
        ...
        OPT(AST_Function, function(self, compressor) {
            tighten_body(self.body, compressor);
            if (compressor.option("unsafe_arrows") && compressor.option("ecma") >= 6 && !self.name && !self.is_generator && !self.uses_arguments && !self.uses_eval) {
        ...
        OPT(AST_ConciseMethod, function(self, compressor) {
            ...
            if (compressor.option("arrows") && compressor.parent() instanceof AST_Object && !self.value.uses_arguments && !self.value.uses_eval && self.value.body.length == 1 && self.value.body[0] instanceof AST_Return && self.value.body[0].value && !self.value.contains_this()) {
